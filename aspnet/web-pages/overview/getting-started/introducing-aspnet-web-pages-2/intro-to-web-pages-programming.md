...
- @Request.UrlОтображает свойства URL-адрес объекта запроса. Объект запроса содержит сведения о текущем запросе из браузера, а само собой свойство URL-адрес содержит URL-адрес текущего запроса.
...
Можно проверить переменные, значения или выражения с помощью *логический оператор* или *оператор сравнения*: равно (==), больше (&gt;), меньше (&lt;), больше или равно (&gt;=) и меньше или равно (&lt;=). ! = Не равно означает оператор — например, если (! = 0) означает *Если* *a**не равно 0*.
