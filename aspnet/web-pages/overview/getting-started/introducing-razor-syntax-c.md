...
*Объект* представляет вещь, вы можете программировать с &#8212; страница, текстовое поле, файл, изображения, веб-запроса, сообщение электронной почты, запись клиента (строки базы данных), и т. д. Объекты имеют свойства, описывающие их характеристики и можно прочитать или изменить &#8212; Объект-поле имеет `Text` свойство (среди прочих), объект запроса имеет `Url` имеет свойства, сообщение электронной почты `From` свойство и объект клиента имеет `FirstName` свойства. Объекты также имеют методы, являющиеся &quot;команд&quot; они могут выполнять. Примеры включают объект файла `Save` метод, объект image `Rotate` метод и объект электронной почты `Send` метод.
...
Синтаксис Razor позволяет добавлять код сервера к содержимому этого клиента. Если на странице кода сервера, сервер выполняет код во-первых, перед отправкой страницы в браузере. Запустив на сервере, код может выполнять задачи, которые могут быть гораздо более сложными, с использованием клиента содержимого сама по себе, например при доступе к серверных баз данных. Самое главное серверный код позволяет динамически генерировать клиента содержимого &#8212; он может создавать разметку HTML или другое содержимое в режиме реального времени и отправьте его в браузер вместе со статическим HTML-страница может содержать. С точки зрения браузера клиент контент, созданный код сервера не отличается от любого другого клиента содержимого. Как вы уже видели, серверный код, необходимый довольно проста.
...
| --- | --- | --- |
| `AsInt(), IsInt()` | Преобразует строку, представляющую целое число (например, «593)» в целое число. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample28.cs)] |
| `AsBool(), IsBool()` | Преобразует строку как &quot;true&quot; или &quot;false&quot; к логическому типу. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample29.cs)] |
| `AsFloat(), IsFloat()` | Преобразует строку, имеет десятичное значение как &quot;1.3&quot; или &quot;7.439&quot; число с плавающей запятой. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample30.cs)] |
| `AsDecimal(), IsDecimal()` | Преобразует строку, имеет десятичное значение как &quot;1.3&quot; или &quot;7.439&quot; в десятичное число. (В ASP.NET, десятичное число, более точно, чем число с плавающей запятой.) | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample31.cs)] |
| `AsDateTime(), IsDateTime()` | Преобразует строку, представляющую значение даты и времени для ASP.NET `DateTime` типа. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample32.cs)] |
| `ToString()` | Любой другой тип данных преобразуется в строку. | [!code-javascript[Main](introducing-razor-syntax-c/samples/sample33.js)] |
...
| --- | --- | --- |
| `+` `-` `*` `/` | Математические операторы, используемые в числовых выражений. | [!code-css[Main](introducing-razor-syntax-c/samples/sample34.css)] |
| `=` | Присваивание. Присваивает значение справа от оператора объекту с левой стороны. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample35.cs)] |
| `==` | Равенство. Возвращает `true` Если значения равны. (Обратите внимание, различие между `=` оператор и `==` оператор.) | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample36.cs)] |
| `!=` | Неравенство. Возвращает `true` Если значения не равны. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample37.cs)] |
| `< > <= >=` | Меньше-чем», «больше — чем меньше или равно», «больше или равно. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample38.cs)] |
| `+` | Объединение, который используется для объединения строк. ASP.NET знает разницу между этот оператор и оператор сложения, основанный на типе данных выражения. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample39.cs)] |
| `+=` `-=` | Операторы инкремента или декремента, которых сложения и вычитания 1 (соответственно) из переменной. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample40.cs)] |
| `.` | Точка. Используются для различения объектов и их свойства и методы. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample41.cs)] |
| `()` | Круглые скобки. Используется для группирования выражений и для передачи параметров в методы. | [!code-javascript[Main](introducing-razor-syntax-c/samples/sample42.js)] |
| `[]` | Квадратные скобки. Используется для доступа к значениям в массивы или коллекции. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample43.cs)] |
| `!` | Не. Обращает `true` значение `false` и наоборот. Обычно используется в качестве это самый простой способ проверки `false` (то есть для не `true`). | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample44.cs)] |
| `&&` <code>&#124;&#124;</code> | Логическое AND и OR, которые используются для связывания условий. | [!code-csharp[Main](introducing-razor-syntax-c/samples/sample45.cs)] |
...
Для массивов объявить определенный тип данных, например `string`, `int`, или `DateTime`. Чтобы указать, что переменная может содержать массив, добавить квадратные скобки в объявлении (такие как `string[]` или `int[]`). Вы может обращаться к элементам в массиве с помощью их положение (индекс) или с помощью `foreach` инструкции. Массив индексов (с нуля) являются &#8212; то есть первый элемент находится в позиции 0, второй элемент находится в позиции 1 и т. д.
...
[Язык C#](https://msdn.microsoft.com/en-us/library/kx37x362.aspx)
