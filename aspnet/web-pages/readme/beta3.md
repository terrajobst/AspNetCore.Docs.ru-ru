...
> <span data-ttu-id="ffaad-127">См. в разделе [пошаговые инструкции для публикации приложений](https://go.microsoft.com/fwlink/?LinkID=196149)</span><span class="sxs-lookup"><span data-stu-id="ffaad-127">See [Step-by-Step Instructions for Publishing Applications](https://go.microsoft.com/fwlink/?LinkID=196149)</span></span>
...
> <span data-ttu-id="ffaad-194">Следующие типы были перемещены `WebMatrix.WebData` сборки:</span><span class="sxs-lookup"><span data-stu-id="ffaad-194">The following types were moved to the `WebMatrix.WebData` assembly:</span></span>
> 
...
### <a name="other-issues"></a><span data-ttu-id="ffaad-347">Другие проблемы</span><span class="sxs-lookup"><span data-stu-id="ffaad-347">Other Issues</span></span>
...
>   
> [<span data-ttu-id="ffaad-383">Приложение, которое запускается с пользователь без прав администратора не может прослушивать HTTP-трафика компьютера, на котором приложение выполняется в Windows Vista, Windows Server 2003 или Windows XP.</span><span class="sxs-lookup"><span data-stu-id="ffaad-383">An application that is started by a non-administrative user cannot listen to the HTTP traffic of the computer on which the application is running in Windows Vista, Windows Server 2003, or Windows XP.</span></span>](https://support.microsoft.com/kb/939786)


#### <a name="issue-google-chrome-is-not-available-as-a-run-option"></a><span data-ttu-id="ffaad-384">Проблема: Google Chrome не поддерживается в случае выполнения</span><span class="sxs-lookup"><span data-stu-id="ffaad-384">Issue: Google Chrome is not available as a Run option</span></span>

> <span data-ttu-id="ffaad-385">Google Chrome не отображается в списке браузеров в разделе **запуска** на **Главная** вкладки.</span><span class="sxs-lookup"><span data-stu-id="ffaad-385">Google Chrome is not displayed in the list of browsers under **Run** on the **Home** tab.</span></span>
> 
> <span data-ttu-id="ffaad-386">**Инструкции по решению**</span><span class="sxs-lookup"><span data-stu-id="ffaad-386">**Workaround**</span></span>  
> <span data-ttu-id="ffaad-387">В некоторых версиях Google Chrome не регистрируют себя правильно с функцией программы по умолчанию в Windows.</span><span class="sxs-lookup"><span data-stu-id="ffaad-387">Some versions of Google Chrome do not register themselves correctly with the Default Programs feature in Windows.</span></span> <span data-ttu-id="ffaad-388">Обойти это ограничение, Google Chrome, выберите пункт *Настройка и управление Google Chrome* меню, щелкните *параметры*, а затем нажмите кнопку *марки Google Chrome обозревателем по умолчанию*.</span><span class="sxs-lookup"><span data-stu-id="ffaad-388">As a workaround, start Google Chrome, click the *Customize and control Google Chrome* menu, click *Options*, and then click *Make Google Chrome my default browser*.</span></span>


#### <a name="issue-the-foreign-key-dialog-box-doesnt-allow-entering-a-primary-key"></a><span data-ttu-id="ffaad-389">Проблема: Диалоговое окно «Foreign Key» не позволяет ввести первичный ключ</span><span class="sxs-lookup"><span data-stu-id="ffaad-389">Issue: The "Foreign Key" dialog box doesn't allow entering a primary key</span></span>

> <span data-ttu-id="ffaad-390">**Foreign Key** диалоговое окно не позволяет ввести имя первичного ключа из таблицы первичного ключа.</span><span class="sxs-lookup"><span data-stu-id="ffaad-390">The **Foreign Key** dialog box does not allow you to enter the primary key name from the primary key table.</span></span>
> 
> <span data-ttu-id="ffaad-391">**Инструкции по решению**</span><span class="sxs-lookup"><span data-stu-id="ffaad-391">**Workaround**</span></span>  
> <span data-ttu-id="ffaad-392">Это сделано намеренно.</span><span class="sxs-lookup"><span data-stu-id="ffaad-392">This is intentional.</span></span> <span data-ttu-id="ffaad-393">Необходимо ввести имя первичного ключа из таблицы первичного ключа.</span><span class="sxs-lookup"><span data-stu-id="ffaad-393">You do not need to enter the name of the primary key from the primary key table.</span></span>


#### <a name="issue-the-relationships-button-is-disabled"></a><span data-ttu-id="ffaad-394">Проблема: Кнопка «Связи» отключена</span><span class="sxs-lookup"><span data-stu-id="ffaad-394">Issue: The "Relationships" button is disabled</span></span>

> <span data-ttu-id="ffaad-395">**Связи** под кнопкой **таблицы** вкладке **баз данных** рабочей области отключен для базы данных SQL Server Compact.</span><span class="sxs-lookup"><span data-stu-id="ffaad-395">The **Relationships** button under the **Table** tab in the **Databases** workspace is disabled for SQL Server Compact databases.</span></span>
> 
> <span data-ttu-id="ffaad-396">**Инструкции по решению**</span><span class="sxs-lookup"><span data-stu-id="ffaad-396">**Workaround**</span></span>  
> <span data-ttu-id="ffaad-397">Отсутствует.</span><span class="sxs-lookup"><span data-stu-id="ffaad-397">None.</span></span> <span data-ttu-id="ffaad-398">SQL Server Compact не поддерживает связи между таблицами.</span><span class="sxs-lookup"><span data-stu-id="ffaad-398">SQL Server Compact does not support relationships between tables.</span></span>


#### <a name="issue-parameterized-sql-queries-throw-exceptions"></a><span data-ttu-id="ffaad-399">Проблема: Параметризованные SQL-запросы исключения</span><span class="sxs-lookup"><span data-stu-id="ffaad-399">Issue: Parameterized SQL queries throw exceptions</span></span>

> <span data-ttu-id="ffaad-400">В SQL Server Compact 4.0, если вы не укажете тип данных таких как `SqlDbType` или `DbType` для параметров в параметризованных запросах, создается исключение при выполнении запроса.</span><span class="sxs-lookup"><span data-stu-id="ffaad-400">In SQL Server Compact 4.0, if you do not specify a data type such as `SqlDbType` or `DbType` for parameters in parameterized queries, an exception is thrown when the query runs.</span></span>
> 
> <span data-ttu-id="ffaad-401">**Инструкции по решению**</span><span class="sxs-lookup"><span data-stu-id="ffaad-401">**Workaround**</span></span>  
> <span data-ttu-id="ffaad-402">Явно задать тип данных для параметров, таких как `SqlDbType` или `DbType`.</span><span class="sxs-lookup"><span data-stu-id="ffaad-402">Explicitly set the data type for parameters such as `SqlDbType` or `DbType`.</span></span> <span data-ttu-id="ffaad-403">Это крайне важно в случае с типами данных больших двоичных ОБЪЕКТОВ (`image` и `ntext`).</span><span class="sxs-lookup"><span data-stu-id="ffaad-403">This is critical in the case of BLOB data types (`image` and `ntext`).</span></span> <span data-ttu-id="ffaad-404">Используйте код, аналогичный следующему:</span><span class="sxs-lookup"><span data-stu-id="ffaad-404">Use code like the following:</span></span>
> 
...
## <a name="for-more-information"></a><span data-ttu-id="ffaad-405">Дополнительные сведения</span><span class="sxs-lookup"><span data-stu-id="ffaad-405">For More Information</span></span>

<span data-ttu-id="ffaad-406">Дополнительные сведения о WebMatrix бета-версии 3 см. в разделе следующих веб-сайтах:</span><span class="sxs-lookup"><span data-stu-id="ffaad-406">For more information about WebMatrix Beta 3, see the following websites:</span></span>

- [<span data-ttu-id="ffaad-407">IIS.net</span><span class="sxs-lookup"><span data-stu-id="ffaad-407">IIS.net</span></span>](http://iis.net/)
- [<span data-ttu-id="ffaad-408">ASP.NET</span><span class="sxs-lookup"><span data-stu-id="ffaad-408">ASP.NET</span></span>](https://asp.net/webmatrix)
- [<span data-ttu-id="ffaad-409">Microsoft.com/web</span><span class="sxs-lookup"><span data-stu-id="ffaad-409">Microsoft.com/web</span></span>](https://www.microsoft.com/web)

* * *

<span data-ttu-id="ffaad-410">© 2010 Microsoft Corporation.</span><span class="sxs-lookup"><span data-stu-id="ffaad-410">© 2010 Microsoft Corporation.</span></span> <span data-ttu-id="ffaad-411">Все права защищены.</span><span class="sxs-lookup"><span data-stu-id="ffaad-411">All Rights Reserved.</span></span> <span data-ttu-id="ffaad-412">[Условия использования](https://msdn.microsoft.cos/cc300389.aspx).</span><span class="sxs-lookup"><span data-stu-id="ffaad-412">[Terms of Use](https://msdn.microsoft.cos/cc300389.aspx).</span></span>
