...
5. Замените содержимое Program.cs кодом из этого примера.
    ...
    - **Программа**, в том числе **Main** метод, определение первичного пути выполнения. В этом методе веб-приложение типа **запуска** запускается на указанный URL-адрес (`http://localhost:8080`). Если для конечной точки требуется безопасности, можно реализовать SSL. См. в разделе [как: Настройка порта SSL-сертификат](https://msdn.microsoft.com/library/ms733791.aspx) Дополнительные сведения.
