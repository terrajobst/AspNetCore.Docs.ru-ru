...
- **Добавление маршрутов для приложения, прежде чем добавлять маршрут концентратора:** Если приложение использует другие маршруты, убедитесь, что добавления маршрута первый вызов `MapHubs`.
...
Эта ошибка возникнет при вызове `MapHubs` не выполняется должным образом. В разделе [как зарегистрировать SignalR маршрута и настройте параметры SignalR](../guide-to-the-api/hubs-api-guide-server.md#route) для получения дополнительной информации.
...
Эта ошибка возникает, если `MapHubs` вызывается дважды для приложения. Некоторые приложения вызывают пример `MapHubs` непосредственно в файле глобального приложения; другим пользователям в вызове в класс-оболочку. Убедитесь, что приложение не оба.
...
Это сделано намеренно. SignalR требуется платформа .NET Framework 4 или более поздней версии; для этого необходимо разработать SignalR приложений в Visual Studio 2010 или более поздней версии.
...
### <a name="web-site-crashes-after-maphubs-call"></a>После вызова MapHubs аварийно завершает работу веб-сайта

Эта проблема исправлена в последней версии SignalR. Убедитесь, что вы используете последней версии SignalR путем обновления установки с помощью NuGet.
...
Разделы, используемые Azure объединительной платы не предназначены для настройки пользователя.
