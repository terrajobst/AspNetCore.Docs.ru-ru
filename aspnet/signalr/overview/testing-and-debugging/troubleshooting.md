...
SignalR использует две модели связи: концентраторы и PersistentConnections. Синтаксис вызова этих моделей связи отличается в клиентском коде. Если вы добавили концентратору в коде сервера, убедитесь, что весь код клиента использует синтаксис правильный концентратора.
...
- **Справочник по адрес прокси-сервера центра неправильный формат:** Эта ошибка часто возникает, если ссылку на созданный концентратор прокси-адрес имеет неправильный формат. Убедитесь, что ссылка на адрес центра создается правильно. См. в разделе [способ создания ссылок динамически формируемых прокси-](../guide-to-the-api/hubs-api-guide-javascript-client.md#dynamicproxy) подробные сведения.
...
- **С помощью IIS 7 или 7.5 без обновления для URL-адреса без расширений приложения:** с помощью IIS 7 или 7.5 требуется обновление для URL-адреса без расширений приложения таким образом, сервер может предоставлять доступ к определениям концентратора в `/signalr/hubs`. Обновление можно найти [здесь](https://support.microsoft.com/kb/980368).
- **IIS кэшировать устарела или поврежден:** чтобы убедиться, что содержимое кэша не устарела, введите следующую команду в окне PowerShell, чтобы очистить кэш:
...
### <a name="protocol-error-unknown-transport-error"></a>«Ошибка протокола: неизвестный транспорта» ошибка
...
### <a name="uncaught-error-signalr-jquery-not-found-please-ensure-jquery-is-referenced-before-the-signalrjs-file-error"></a>«Никаким кодом не перехватывается ошибка: SignalR: не найден jQuery. Ошибка, убедитесь, что существует jQuery, прежде чем файл SignalR.js»
...
### <a name="invalidoperationexception-not-a-valid-web-socket-request"></a>«InvalidOperationException: не запрос допустимый веб-сокета.
