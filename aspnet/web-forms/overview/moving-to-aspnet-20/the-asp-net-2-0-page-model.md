...
Возвращает путь относительно приложения для страницы или элемента управления. Например, для страницы, расположенный в http://app/folder/page.aspx, свойство возвращает ~ / folder /.

## <a name="apprelativevirtualpath"></a>AppRelativeVirtualPath

Возвращает относительный виртуальный путь к каталогу страницы или элемента управления. Например для страницы, расположенный в http://app/folder/page.aspx, свойство возвращает ~ / folder/page.aspx.
...
| **GetCallbackEventReference** | См. выше в этом модуле обратные вызовы из сценария. |
| --- | --- |
| **GetPostBackClientHyperlink** | Возвращает ссылку JavaScript (javascript:&lt;вызовите&gt;) может использоваться для отправки обратно из событий на стороне клиента. |
| **GetPostBackEventReference** | Возвращает строку, который может использоваться для запуска отправки обратно клиенту. |
| **GetWebResourceUrl** | Возвращает URL-адрес ресурса, внедренного в сборку. Необходимо использовать в сочетании с **RegisterClientScriptResource**. |
| **RegisterClientScriptResource** | Регистрирует страницу веб-ресурса. Это встроенный в сборку и может быть обработано новый дескриптор WebResource.axd ресурсов. |
| **RegisterHiddenField** | Регистрирует скрытое поле формы со страницей. |
| **RegisterOnSubmitStatement** | Регистрирует клиентский код, который выполняется при отправке формы HTML. |
