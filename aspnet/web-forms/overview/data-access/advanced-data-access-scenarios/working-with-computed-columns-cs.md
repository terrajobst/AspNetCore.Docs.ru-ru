...
**Рис. 5**: Переименовать `GetData` метод `GetSuppliers` ([Просмотр полноразмерного изображения](working-with-computed-columns-cs/_static/image15.png))
...
**Рис. 10**: имя методов класса TableAdapter `FillBySupplierID` и `GetSupplierBySupplierID` ([Просмотр полноразмерного изображения](working-with-computed-columns-cs/_static/image30.png))
...
**Рис. 12**: каждого поставщика, перечисленных в сетке ([Просмотр полноразмерного изображения](working-with-computed-columns-cs/_static/image36.png))
