...
Дополнительные сведения о вычисляемых столбцов в Microsoft SQL Server, см. [технической документации](https://msdn.microsoft.com/library/ms191250.aspx). Ознакомьтесь также с [как: Specify Computed Columns](https://msdn.microsoft.com/library/ms188300.aspx) Пошаговое руководство по созданию вычисляемых столбцов.
...
**Рис. 5**: Переименовать `GetData` метод `GetSuppliers` ([Просмотр полноразмерного изображения](working-with-computed-columns-vb/_static/image15.png))
...
**Рис. 10**: имя методов класса TableAdapter `FillBySupplierID` и `GetSupplierBySupplierID` ([Просмотр полноразмерного изображения](working-with-computed-columns-vb/_static/image30.png))
...
**Рис. 12**: каждого поставщика, перечисленных в сетке ([Просмотр полноразмерного изображения](working-with-computed-columns-vb/_static/image36.png))
