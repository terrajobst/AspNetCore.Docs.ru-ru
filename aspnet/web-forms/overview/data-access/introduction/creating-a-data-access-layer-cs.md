...
На следующем экране **InsertCommand** **CommandText** отображается. Расширить этот запрос, добавив **Выбор области\_IDENTITY()** в конце запроса, который возвращает последнее значение идентификатора, вставленное в **УДОСТОВЕРЕНИЕ** столбец в той же области. (См. [технической документации](https://msdn.microsoft.com/library/ms190315.aspx) Дополнительные сведения о **область\_IDENTITY()** и поэтому может потребоваться [использовать область\_IDENTITY() вместо @ @IDENTITY](http://weblogs.sqlteam.com/travisl/archive/2003/10/29/405.aspx).) Убедитесь, что завершить **вставить** инструкция точкой с запятой перед добавлением **ВЫБЕРИТЕ** инструкции.
