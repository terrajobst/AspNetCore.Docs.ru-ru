...
> *> [!NOTE] Если производится на Windows Vista с помощью пользователя доступа (UAC) включен, и у вас есть Visual Studio 2008, для запуска с правами администратора, Visual Studio не сможет присоединиться к процессу, когда будет предложено присоединить. Чтобы обойти эту проблему, сначала запустите Visual Studio и отладка с помощью этого экземпляра.*
...
> *> [!NOTE] Если не отображается в меню обозревателя скриптов в списке, перейдите в меню* *Настройка* *команды в меню Visual Studio .NET. Найдите запись отладки в разделе «категории» и щелкните его, чтобы отобразить все доступные меню записи. В списке команд, прокрутите вниз, чтобы обозреватель скриптов, а затем перетащите его на отладочные* *меню Windows в упомянутых ранее. Таким образом будет доступна запись меню обозревателя скриптов каждый раз при запуске Visual Studio .NET.*
...
**Рис. 6**: диалоговое окно ошибки скрипта Web Development Helper ([Просмотр полноразмерного изображения](understanding-asp-net-ajax-debugging-capabilities/_static/image18.png))
...
**Рис. 8**: UpdatePanel ответное сообщение просматривать с помощью средства просмотра журнала HTTP.  ([Просмотр полноразмерного изображения](understanding-asp-net-ajax-debugging-capabilities/_static/image24.png))
...
**Рис. 10**: инспектор DOM с помощью Firebug.  ([Просмотр полноразмерного изображения](understanding-asp-net-ajax-debugging-capabilities/_static/image30.png))
...
**Рис. 12**: Sys.Debug.assert сообщение о сбое.  ([Просмотр полноразмерного изображения](understanding-asp-net-ajax-debugging-capabilities/_static/image36.png))
