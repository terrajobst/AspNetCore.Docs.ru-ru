...
ms.locfileid: "37393065"
---
<a name="understanding-aspnet-ajax-authentication-and-profile-application-services"></a>Основные сведения о проверки подлинности AJAX ASP.NET и служб профилирования приложений
====================
...
| URL-адрес перенаправления | Необязательно (по умолчанию — null). URL-адрес для перенаправления браузера, чтобы после успешной проверки подлинности. Если этот параметр имеет значение null или является пустой строкой, происходит перенаправление. |
...
| **Имя параметра** | **Значение** |
| --- | --- |
| результат | Этот параметр всегда будет иметь `null`; он зарезервирован для использования в будущем. |
| userContext | Указывает сведения о контексте пользователя, при вызове функции входа в систему. |
| methodName | Имя вызывающего метода. |
