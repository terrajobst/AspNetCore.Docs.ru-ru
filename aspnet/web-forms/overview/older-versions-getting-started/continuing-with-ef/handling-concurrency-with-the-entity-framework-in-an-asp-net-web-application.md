...
- Можно хранить список какие свойства были изменены пользователем и обновлять соответствующие столбцы в базе данных. В примере сценария данные не будут потеряны, поскольку различные свойства были обновлены двумя пользователями. Далее время кто-то просматривает журнал отдела, появится 1/1/1999 и 125,000.00 $. 
...
- Можно перезаписать изменения Джона изменение Джейн. После нажатия кнопки Мария **обновление**, **бюджета** сумма возвращается к 1,000,000.00 $. Это называется *клиент побеждает* или *побеждает последний* сценария. (Значения клиента имеют приоритет над возможности хранилища данных).
...
На этом завершается Общие сведения для обработки конфликтов параллелизма. Следующее руководство будут представлены рекомендации по повышению производительности веб-приложения, использующего платформу Entity Framework.
