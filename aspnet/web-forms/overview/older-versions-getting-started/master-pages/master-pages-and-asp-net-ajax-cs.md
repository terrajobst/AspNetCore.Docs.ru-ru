...
[Загрузить код](http://download.microsoft.com/download/1/8/4/184e24fa-fcc8-47fa-ac99-4b6a52d41e97/ASPNET_MasterPages_Tutorial_08_CS.zip) или [скачать PDF](http://download.microsoft.com/download/e/b/4/eb4abb10-c416-4ba4-9899-32577715b1bd/ASPNET_MasterPages_Tutorial_08_CS.pdf)
...
> [!NOTE]
> Этот учебник не исследовать, разработки и построения с поддержкой AJAX веб-приложений с помощью платформы ASP.NET AJAX. Дополнительные сведения об использовании AJAX можно найти [видео ASP.NET AJAX](../../../videos/aspnet-ajax/index.md) и [учебники](../aspnet-ajax/understanding-partial-page-updates-with-asp-net-ajax.md), а также как эти ресурсы, перечисленные в разделе "Дополнительные материалы" в конце этого учебника.
...
Откройте `Site.master` главную страницу и добавление элемента управления ScriptManager страницы в веб-форму, но перед `<div id="topContent">` элемент (см. рис. 1). При использовании Visual Web Developer 2008 или Visual Studio 2008, на панели инструментов на вкладке расширения AJAX расположен элемент управления ScriptManager. Если вы используете Visual Studio 2005, необходимо сначала установить платформу ASP.NET AJAX и добавьте элементы управления панели элементов. Посетите [вики-сайте ASP.NET AJAX](https://github.com/DevExpress/AjaxControlToolkit/wiki) для получения платформа ASP.NET 2.0.
...
**На рисунке 01**: Добавление ScriptManager на главную страницу ([Просмотр полноразмерное изображение](master-pages-and-asp-net-ajax-cs/_static/image3.png))
...
[![Добавление новой страницы ASP.NET на веб-сайт](master-pages-and-asp-net-ajax-cs/_static/image5.png)](master-pages-and-asp-net-ajax-cs/_static/image4.png)

**На рисунке 02**: Добавление новой страницы ASP.NET на веб-сайт ([Просмотр полноразмерное изображение](master-pages-and-asp-net-ajax-cs/_static/image6.png))


Помните, что в [ *указанием заголовка, мета-теги и другие заголовки HTML на главной странице* ](specifying-the-title-meta-tags-and-other-html-headers-in-the-master-page-cs.md) учебник, созданную пользовательскую страницу базовый класс с именем `BasePage` , создавшей заголовок страницы, если она была не задан явно. Последовательно выберите пункты `ShowRandomProduct.aspx` на странице кода класса и его производные от `BasePage` (вместо из `System.Web.UI.Page`).
...
[![Привязать DetailsView для нового элемента управления SqlDataSource](master-pages-and-asp-net-ajax-cs/_static/image8.png)](master-pages-and-asp-net-ajax-cs/_static/image7.png)

**На рисунке 03**: привязать новый элемент управления SqlDataSource DetailsView ([Просмотр полноразмерное изображение](master-pages-and-asp-net-ajax-cs/_static/image9.png))


Настройка для подключения к базе данных Northwind с помощью элемента управления SqlDataSource `NorthwindConnectionString` (который мы создали в [ *взаимодействия с главной страницы на странице содержимого* ](interacting-with-the-content-page-from-the-master-page-cs.md) учебника). При настройке инструкции select решили указать пользовательские инструкции SQL, а затем введите следующий запрос:
...
**На рисунке 04**: Настройка SqlDataSource для возврата в один случайным образом выбрать записи ([Просмотр полноразмерное изображение](master-pages-and-asp-net-ajax-cs/_static/image12.png))
...
[![Отображается имя и цену случайных продукта](master-pages-and-asp-net-ajax-cs/_static/image14.png)](master-pages-and-asp-net-ajax-cs/_static/image13.png)

**На рисунке 05**: отображается название и цену продукта произвольного ([Просмотр полноразмерное изображение](master-pages-and-asp-net-ajax-cs/_static/image15.png))
...
[![При загрузке страницы отображается случайным образом выбраны продукта](master-pages-and-asp-net-ajax-cs/_static/image17.png)](master-pages-and-asp-net-ajax-cs/_static/image16.png)

**На рисунке 06**: случайно выбранных продуктов отображается при загрузке страницы ([Просмотр полноразмерное изображение](master-pages-and-asp-net-ajax-cs/_static/image18.png))
...
[![Добавьте новый файл JavaScript в папку «скрипты»](master-pages-and-asp-net-ajax-cs/_static/image23.png)](master-pages-and-asp-net-ajax-cs/_static/image22.png)

**На рисунке 08**: добавьте новый файл JavaScript, чтобы `Scripts` папку ([Просмотр полноразмерное изображение](master-pages-and-asp-net-ajax-cs/_static/image24.png))


[![Веб-сайт будет добавлен новый файл JavaScript](master-pages-and-asp-net-ajax-cs/_static/image26.png)](master-pages-and-asp-net-ajax-cs/_static/image25.png)

**На рисунке 09**: веб-сайт будет добавлен новый файл JavaScript ([Просмотр полноразмерное изображение](master-pages-and-asp-net-ajax-cs/_static/image27.png))


Добавьте следующий скрипт в файл TimerScript.js.
...
[![Добавьте ссылку на скрипт прокси диспетчера скриптов](master-pages-and-asp-net-ajax-cs/_static/image29.png)](master-pages-and-asp-net-ajax-cs/_static/image28.png)

**Рис. 10**: добавить ссылку на скрипт прокси диспетчера скриптов ([Просмотр полноразмерное изображение](master-pages-and-asp-net-ajax-cs/_static/image30.png))
...
**Рис. 11**: нажмите эту кнопку для остановки элемента управления Timer ([Просмотр полноразмерное изображение](master-pages-and-asp-net-ajax-cs/_static/image33.png))


[![Нажмите кнопку "Продолжить" для перезапуска таймера](master-pages-and-asp-net-ajax-cs/_static/image35.png)](master-pages-and-asp-net-ajax-cs/_static/image34.png)

**Рис. 12**: нажмите кнопку "Продолжить" для перезапуска таймера ([Просмотр полноразмерное изображение](master-pages-and-asp-net-ajax-cs/_static/image36.png))
