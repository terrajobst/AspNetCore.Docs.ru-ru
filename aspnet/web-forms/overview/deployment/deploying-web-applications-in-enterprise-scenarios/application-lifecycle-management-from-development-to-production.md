...
> 
> Раздел предназначен для предоставления высокоуровневый обзор для [серию учебников,](deploying-web-applications-in-enterprise-scenarios.md) на веб-развертывания в организации. Не беспокойтесь, если вы не знакомы с некоторыми из описанные здесь принципы & #x 2014, учебники, выполните предоставляют подробные сведения о всех этих задачах и методиках разработки.
> 
> > [!NOTE]
...
> В каждом случае конечным результатом является таким же & #x 2014; MSBuild выполняет файл объединенных проекта и развертывание решения в целевой среде. Это обеспечивает большую гибкость в том, как запустить процесс публикации.
...
В пределах *Publish.proj* файл **WhatIf** свойство указывает, что все ресурсы развертывания должны публиковаться в режиме «что, если». Другими словами файлы журнала создаются, как если развертывание вышли вперед, но фактически ничего не изменяется в целевой среде. Это позволяет оценить влияние развертывания предложенных & #x 2014, в частности, что будет Станьте, что будет обновляться и что будут удалены & #x 2014; прежде чем фактически внести изменения.
