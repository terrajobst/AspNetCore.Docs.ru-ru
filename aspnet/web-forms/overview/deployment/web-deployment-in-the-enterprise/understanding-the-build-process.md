...
Первое, что происходит — файлы два проекта&#x2014;, содержащий универсальные инструкции сборки и развертывания, а также, содержащий параметры, относящиеся к среде&#x2014;объединяются в один файл проекта. Затем MSBuild работают, минуя инструкции в файле проекта. Он создает все проекты в решении, с использованием файла проекта для каждого проекта. Затем вызывается для других средств, таких как веб-развертывания (MSDeploy.exe) и служебную программу VSDBCMD для развертывания веб-содержимого и баз данных в целевой среде.
...
2. Он создает каждый проект в решении:
    ...
    1. Для веб-проектов&#x2014;таким образом, веб-приложение ASP.NET MVC и WCF веб-службы&#x2014;процесс построения создает пакет веб-развертывания для каждого проекта.
    2. Для проектов баз данных в процесс построения создает манифест развертывания (DEPLOYMANIFEST-файл) для каждого проекта.
3. Она использует служебную программу VSDBCMD.exe развертывание каждого проекта базы данных в решении, с помощью различных свойств в файлы проекта&#x2014;целевая строка подключения и имя базы данных&#x2014;вместе с DEPLOYMANIFEST-файл.
...
MSBuild обрабатывает эту инструкцию, построение списка элементов с именем **ProjectsToBuild**. В этом случае в списке элементов содержит одно значение&#x2014;путь и имя файла решения.

На этом этапе оставшиеся элементы, целевые объекты. Целевые объекты обрабатываются по-разному из свойства и элементы&#x2014;по существу, целевые объекты, не обрабатываются так, если они являются явно заданное пользователем или вызван другой конструкции в файле проекта. Помните, что открывающий **проекта** тег содержит **DefaultTargets** атрибута.
...
> [!NOTE]
> **Удостоверение** является одним из [встроенные метаданные значения](https://msdn.microsoft.com/library/ms164313.aspx) , которое присваивается при создании каждого элемента. Он ссылается на значение **Include** атрибута в **элемент** элемент&#x2014;иными словами, путь и имя файла элемента.
