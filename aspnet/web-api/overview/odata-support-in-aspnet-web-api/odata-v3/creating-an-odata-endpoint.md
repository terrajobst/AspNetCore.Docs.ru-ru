...
Первый параметр — понятное имя для маршрута. Клиенты службы, это имя не отображается. Второй параметр — префикс URI для конечной точки. Этот код, URI для набора сущностей Products задан http://<em>hostname</em>  /odata и продуктов. Приложение может иметь более одной конечной точки OData. Для каждой конечной точки, вызовите <strong>MapODataRoute</strong> и укажите имя уникального маршрута и уникальный префикс URI.
