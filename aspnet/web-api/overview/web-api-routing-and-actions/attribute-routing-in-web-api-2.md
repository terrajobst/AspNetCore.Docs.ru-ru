...
| --- | --- | --- |
| альфа-канала | Совпадения прописные или строчные буквы латинского алфавита (a-z, A-Z) | {x: альфа-канал} |
...
### <a name="custom-route-constraints"></a>Ограничения пользовательских маршрутов
...
Когда платформа пытается соответствует URI с маршрутом, она проверяет маршруты в определенном порядке. Чтобы указать порядок, задайте **RouteOrder** атрибута маршрута. Более низкие значения вычисляются первыми. Значение порядка по умолчанию равно нулю.
...
Обратите внимание, что «подробности» является сегментом литерала и появляется перед «{id}», но «Ожидание» отображается последнее, так как **RouteOrder** свойства является 1. (В этом примере предполагается, что существует то заказчики с именем «подробности» или «Ожидание». Как правило Старайтесь избегать неоднозначных маршрутов. В этом примере лучше шаблон маршрута для `GetByCustomer` является «клиентов / {customerName}»)
