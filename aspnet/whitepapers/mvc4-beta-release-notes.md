...
- **Согласование содержимого**: клиент и сервер могут работать совместно для определения корректный формат данных, возвращаемых из API. Мы предоставляем поддержку по умолчанию для XML, JSON и форматы форму URL-адреса и расширения этой поддержки, добавив собственные модули форматирования, или даже заменить стратегии согласование содержимого по умолчанию.
- **Привязка моделей и проверки:** связыватели моделей обеспечивают простой способ извлечения данных из различных частей HTTP-запроса и преобразования этих частей сообщения в объекты .NET, которые могут использоваться при выполнении действий веб-API.
...
- **Построении запроса:** путем простого возврата IQueryable&lt;T&gt;, веб-API будет поддерживать запросов через соглашения URL-адрес OData.
...
- **Улучшенная Inversion of Control (IoC) с помощью DependencyResolver:** веб-API теперь использует схемы обнаружения службы, реализуемый Сопоставитель зависимостей MVC для получения экземпляров для многих различных средств.
- **Конфигурация на основе кода:** Настройка веб-API выполняется исключительно с помощью кода, оставив файл config очистить файлы.
