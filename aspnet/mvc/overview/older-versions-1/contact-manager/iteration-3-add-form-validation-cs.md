...
---
<a name="iteration-3--add-form-validation-c"></a>Итерации #3 – добавить проверку формы (C#)
====================
по [Microsoft](https://github.com/microsoft)

[Загрузить исходный код](iteration-3-add-form-validation-cs/_static/contactmanager_3_cs1.zip)
...
[![Диалоговое окно нового проекта](iteration-3-add-form-validation-cs/_static/image1.jpg)](iteration-3-add-form-validation-cs/_static/image1.png)

**На рисунке 01**: формы проверки подлинности ([Просмотр полноразмерное изображение](iteration-3-add-form-validation-cs/_static/image2.png))
...
Обратите внимание, вызов вспомогательный метод Html.ValidationSummary(), который появляется сразу над HTML-формы. Если имеются ошибки проверки, этот метод отображает сообщения проверки в виде маркированного списка.

Обратите внимание на то, кроме того, вызовы Html.ValidationMessage(), отображаемые рядом с каждым полем формы. Вспомогательный объект ValidationMessage() отображает сообщение об ошибке отдельных проверки. В случае список 1 появится звездочка при ошибке проверки.

Наконец Html.TextBox() вспомогательное приложение автоматически отображает класс каскадной таблицы стилей при ошибке проверки, связанные со свойством, отображается вспомогательный метод. Подготавливает к просмотру Html.TextBox() вспомогательный класс с именем **ошибка для проверки входных данных**.

При создании нового приложения ASP.NET MVC, таблицу стилей с именем Site.css автоматически создается в папке содержимого. Эта таблица стилей содержит следующие определения классов CSS, связанные с внешним видом сообщений об ошибках проверки.
...
**Листинг 2 - Controllers\ContactController.cs (создать проверки)**
