...
- *WebAPIpgw*: проекта веб-API ASP.NET MVC 4, который реализует `Products, Gizmos and Widgets` контроллеров. Он предоставляет данные для *WebAppAsync* проекта и *Mvc4Async* проекта.
- *WebAppAsync*: проекта веб-формы ASP.NET, используемого в следующем учебнике.
