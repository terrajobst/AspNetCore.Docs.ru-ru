...
Выглядит довольно хорошо. Обратите внимание, что в строке заголовка браузера отображается &quot;индекса — Мои приложения ASP.NET» и больших ссылку верхней части страницы появляется надпись «Имя приложения». В зависимости от того, о том, какие внести окно браузера, может потребоваться щелкните три полосы в правом верхнем углу, чтобы просмотреть для **Главная**, **о**, **контакт**, **Зарегистрировать** и **вход** ссылки.
...
Контроллеры — отвечает за предоставление независимо от используемых данных и объектов необходимых для шаблона представления для отображения ответа в браузер. Рекомендуется: **шаблона представления никогда не следует выполнять бизнес-логику или напрямую взаимодействовать с базой данных**. Вместо этого они должны работать только с данными, предоставляется к ней с помощью контроллера. Для обслуживания таких &quot;Разделение областей ответственности&quot; позволяет поддерживать код очистки, пригодного для тестирования и поддержки.
