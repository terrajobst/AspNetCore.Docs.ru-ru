...
При разработке нового приложения модель данных меняется часто и каждый раз изменения модели, она возвращает синхронизации с базой данных. Вы настроили для автоматического удаления и повторного создания базы данных каждый раз при изменении модели данных Entity Framework. При добавлении, удалить, или изменить классы сущностей или изменить ваш `DbContext` класса, при очередном запуске приложения автоматически удаляет существующей базы данных, создает новый, который соответствует модели и заполняет его с тестовыми данными.
...
2. Также в приложении *Web.config* файл, измените имя базы данных в строке соединения на ContosoUniversity2.
    ...
    Это изменение настраивает проект, чтобы первый миграции создает новую базу данных. Это необязательно, но вы увидите Далее почему он имеет смысл.
...
- Используйте **обозревателя серверов** для проверки базы данных как в первом руководстве и запустите приложение, чтобы убедиться, что все данные по-прежнему работает так же, как и раньше.
...
12. Нажмите кнопку **создать** в нижней части колонки для указания, что по завершении вы сможете.
...
7. После проверки подключения отображается рядом с зеленой галочкой **проверить подключение** кнопки. Нажмите кнопку **Далее**.
...
8. Откройте **строку подключения к удаленному** раскрывающегося списка в разделе **SchoolContext** и выберите строку подключения для базы данных, вы создали.
9. Выберите **обновление базы данных**.
