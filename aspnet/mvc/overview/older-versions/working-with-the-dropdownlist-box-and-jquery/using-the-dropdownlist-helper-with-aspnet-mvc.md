...
Изменить список выбора для строки "Драма" и нажмите клавишу **отправить** кнопки. URL-адрес в браузере `http://localhost:2468/Home/CategoryChosen?MovieType=1` и на странице отображается **выбрано: 1**.
...
В приведенном выше коде мы добавим атрибут HTML и значение атрибута `class = "chzn-select"`. Символ @ перед класс никак не с помощью представлений Razor. `class` — [ключевое слово C#](https://msdn.microsoft.com/library/x53a06bb.aspx). Ключевые слова C# не может использоваться как идентификаторы, если они содержат префикс. В примере выше `@class` является допустимым идентификатором, но **класс** не, так как **класс** является ключевым словом.
