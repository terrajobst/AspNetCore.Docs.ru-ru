...
Если вы не знакомы с фрагменты кода Visual Studio и хотите научиться использовать их, можно ссылаться в приложение из этого документа &quot; [фрагменты кода с помощью приложении A:](#AppendixA)&quot;.
...
> [!NOTE]
> — Базовый формат носителя запроса: @media \[область: все | handheld | печати | проекции | экрана\] ([свойство: значение] и... [свойство: значение])
...
> <strong>@media экран и (min нулевой ширины: 400 пикселей) и (максимальная ширина: 700px) {...}:</strong> только для экранов. Разрешение должно быть от 400 до 700px.
> 
> <strong>@media портативные и (min нулевой ширины: 20em), экран и (min нулевой ширины: 20em) {...}:</strong> для экранов и карманных устройств (mobile и устройств). Минимальная ширина должна быть больше 20em.
> 
...
3. Откройте <strong>Site.css</strong> файл из обозревателя решений Visual Studio, находятся в <strong>содержимого</strong> папки проекта. Нажмите клавишу <strong>CTRL + F</strong> открытия Visual Studio интегрированный поиск, и записи <strong>@media</strong> искомого <strong>медиа-запросов CSS</strong>.
    ...
    Условие запроса мультимедиа, определенные в этот шаблон работает таким образом: если меньше размер окна браузера **850 пикселей**, применены правила CSS, они определены внутри этого блока мультимедиа.
...
6. Найдите элемент div с атрибутом роли данных, равным **страницы** и измените **data-theme** атрибут &quot; ****&quot;.
...
## <a name="appendix-c-installing-webmatrix-2-and-iphone-simulator"></a>Приложение c. Установка WebMatrix 2 и iPhone симулятор
...
2. Щелкните стрелку рядом с «־עךנע» и выберите **просмотр с помощью**.
