...
Обновленный проект углового шаблон предоставляет удобную начальную точку для ASP.NET Core приложения для угловой и углового CLI для реализации полнофункционального клиентского пользовательского интерфейса (UI).
...
При использовании ASP.NET Core 2.0, убедитесь, вы уже [установленного шаблона обновленный проект углового](xref:spa/index#installation). Если у вас есть ASP.NET Core 2.1, нет необходимости для ее установки.
...
Запустите [dotnet запуска](/dotnet/core/tools/dotnet-run) для запуска приложения. Регистрируется сообщение, аналогичное следующему:
