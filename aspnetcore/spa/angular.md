...
Перейдите к следующему URL-адресу в браузере.
...
При запуске приложения ASP.NET Core, сервер Angular CLI запущен не будет. Вместо этого будет использоваться экземпляр, запущенный вручную. Это позволит быстрее производить запуск и перезапуск приложения. Теперь не придется ждать каждый раз, когда Angular CLI перестроит клиентское приложение.
