...
Приложение запускает экземпляр сервера Angular CLI в фоновом режиме. Следующее сообщение будет записано в журнал. А *Live&lt;Development Server прослушивает localhost: осерпорт&gt;, откройте браузер на http://localhost:&lt компьютере; осерпорт&gt;/* . Игнорируйте это сообщение — этот URL-адрес **не соответствует** объединенному приложению ASP.NET Core и Angular CLI.
