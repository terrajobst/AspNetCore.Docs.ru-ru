...
Параметр проверки подлинности был добавлен к шаблонам проектов "**angular**" и "**react**", похожим на параметр проверки подлинности в **веб-приложении (модель-представление-контроллер)** (MVC) и **веб-приложении** (Razor Pages) шаблоны проектов. Допустимые значения параметра: **None** и **индивидуальных**. Шаблон проекта **React.js и Redux** в настоящее время не поддерживает параметр проверки подлинности.
...
В файле *controllers\sampledatacontroller.cs* Обратите внимание на `[Authorize]` атрибут, примененный к классу, который указывает, что пользователь должен быть полномочным, исходя из политики по умолчанию для доступа к ресурсу. Политика авторизации по умолчанию должна быть настроена для использования схемы проверки подлинности по умолчанию, `AddIdentityServerJwt` которая настроена в упомянутой выше схеме политики, `JwtBearerHandler` что делает настройку, настроенную таким вспомогательным методом, обработчиком по умолчанию для запросы к приложению.

### <a name="applicationdbcontext"></a>ApplicationDbContext

В файле *data\applicationdbcontext.cs* Обратите внимание на то `DbContext` же, что используется в удостоверении с тем исключением, которое оно расширяет `ApiAuthorizationDbContext` ( `IdentityDbContext`более производным классом от), чтобы включить схему для IdentityServer.
...
В файле *controllers\oidcconfigurationcontroller.cs* Обратите внимание на конечную точку, подготовленную для обслуживания параметров OIDC, которые необходимо использовать клиенту.
...
## <a name="general-description-of-the-angular-app"></a>Общее описание Angular приложения

Поддержка проверки подлинности и авторизации API в угловом шаблоне находится в своем собственном вспомогательном модуле в каталоге *clientapp\src\api-authorization* . Модуль состоит из следующих элементов:
...
## <a name="general-description-of-the-react-app"></a>Общее описание React приложения
