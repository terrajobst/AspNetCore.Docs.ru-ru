...
Зарегистрированным пользователям вашего веб-сайта. [IdentityUser](/dotnet/api/microsoft.aspnet.identity.corecompat.identityuser) типа может расширенные или использовать в качестве примера для пользовательского типа. Нет необходимости наследовать от определенного типа для реализации решения хранилища пользовательское удостоверение.
...
Группы авторизации для веб-узла. Содержит имя роли идентификатор и роли (например, «Admin» или «Employee»). [Пример](/dotnet/api/microsoft.aspnet.identity.corecompat.identityrole)
...
Инкапсулирует сведения для подключения к вашей механизм сохранения и выполнения запросов. Несколько классов данных требуют экземпляр этого класса обычно предоставляются через внедрения зависимостей. [Пример](/dotnet/api/microsoft.aspnet.identity.corecompat.identitydbcontext-1).
...
Хранит и извлекает сведения об имени входа пользователя (например, внешний поставщик аутентификации). [Пример](/dotnet/api/microsoft.aspnet.identity.corecompat.userstore-1)

### <a name="userrole-storage"></a>Объем хранилища UserRole

Хранит и извлекает назначенных ролей для пользователей. [Пример](/dotnet/api/microsoft.aspnet.identity.corecompat.userstore-1)
...
При реализации поставщика хранилища, создание пользовательского класса, который эквивалентен [ `IdentityUser` класса](/dotnet/api/microsoft.aspnet.identity.corecompat.identityuser).
