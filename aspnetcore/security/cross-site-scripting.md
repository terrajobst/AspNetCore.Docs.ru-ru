...
---
# <a name="prevent-cross-site-scripting-xss-in-aspnet-core"></a>Предотвращения межсайтовых скриптов (XSS) в ASP.NET Core
...
>[!WARNING]
> Основные ASP.NET MVC предоставляет `HtmlString` класс, которой не автоматически после выходных данных. Это никогда не можно использовать в сочетании с ненадежных входных данных, при этом будут представлены уязвимость XSS.

## <a name="javascript-encoding-using-razor"></a>Кодирование JavaScript с помощью Razor

Могут возникнуть ситуации, нужно вставить значение в JavaScript для обработки в представлении. Это можно сделать двумя способами. Наиболее безопасным способом для вставки значений — поместить значение в атрибуте данных тега и поиск его в JavaScript. Пример:
...
Проверка может быть полезным инструментом ограничение атак с XSS. Например числовые строка, содержащая только символы 0-9 не активирует XSS атаки. Проверка становится более сложным, следует нужно принимать HTML во вводимых - синтаксического анализа HTML-ввода является сложным, невозможно. Другие форматы текста и разметки будет более безопасный параметр для форматированного ввода. Никогда не следует полагаться на проверку сама по себе. Всегда кодировать недоверенные входные данные перед выводом, независимо от того, какая проверка выполнена.
