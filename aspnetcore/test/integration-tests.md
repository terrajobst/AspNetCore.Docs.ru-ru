...
2. Используйте пользовательский `CustomWebApplicationFactory` в тестовых классах. В следующем примере используется фабрика в классе `IndexPageTests`:
   ...
   Клиент примера приложения настроен так, чтобы запретить `HttpClient` от следующих перенаправлений. Как описано далее в разделе « [Макет проверки подлинности](#mock-authentication) », это позволяет проверкам результатов первого ответа приложения. Первый ответ — это перенаправление во многих из этих тестов с заголовком `Location`.
