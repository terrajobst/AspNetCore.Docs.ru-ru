...
`DataType` Атрибут выдает HTML 5 `data-` (произносится данных тире) атрибутов, которые потребляют браузеров HTML 5. `DataType` Атрибуты не обеспечивают проверку.

`DataType.Date`не указывает формат отображения даты. По умолчанию, поле «Дата» отображается согласно форматы по умолчанию на сервере [CultureInfo](https://docs.microsoft.com/aspnet/core/fundamentals/localization#provide-localized-resources-for-the-languages-and-cultures-you-support).
...
Например если `Department.InstructorID` свойство не было определено как допускающая значение NULL:

* EF Core настраивает правила cascade delete для удаления инструктора при удалении подразделения.
* Удаление инструктора при удалении отделе режим не используется.
...
* Несколько строк для одной и той же инструктора и курса не является допустимым.
