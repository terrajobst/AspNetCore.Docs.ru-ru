...
`AsNoTracking` Метод повышает производительность в сценариях, где сущностей, возвращаемых не обновляется в течение срока службы в текущем контексте. Дополнительные сведения о `AsNoTracking` в конце этого учебника.
...
Запустите страницу, выбрав **учащихся** и нажав кнопку **создать новый**.
...
* `Added`. Сущности в базе данных еще не существует. `SaveChanges` Метод выдает инструкции INSERT.
...
Запустите приложение и выберите **учащихся** , затем щелкните **изменить** гиперссылки.
