...
В предыдущей главе вы создали локальный репозиторий Git для приложения для чтения простого веб-канала. В этой главе будут публиковать этого кода в репозитории GitHub, а также создание конвейера DevOps в Visual Studio Team Services (VSTS). Конвейер позволяет непрерывного выполнения сборки и развертывания приложения. Любой фиксации в репозиторий GitHub активирует сборку и развертывание в промежуточный слот веб-приложения Azure.
...
* Создание учетной записи VSTS
* Создание командного проекта в VSTS
...
Удалите локальное развертывание Git, выполнив следующие действия. VSTS заменяет и расширяет эту функциональность.
...
## <a name="create-a-vsts-account"></a>Создание учетной записи VSTS

1. Откройте браузер и перейдите к [страницу создания учетной записи VSTS](https://go.microsoft.com/fwlink/?LinkId=307137).
1. Введите уникальное имя в **выберите запоминающееся имя** textbox для формирования URL-адрес для доступа к учетной записи VSTS.
...
1. Откройте по электронной почте подтверждение, указывающее на то, что учетная запись VSTS и проекта будут готовы к использованию. Нажмите кнопку **запустить проект** кнопки:
...
## <a name="configure-the-devops-pipeline"></a>Настройка конвейера DevOps
...
### <a name="grant-vsts-access-to-the-github-repository"></a>Предоставление доступа VSTS в репозиторий GitHub
...
1. Требуется авторизация для доступа к репозиторию GitHub VSTS. Введите *< GitHub_username > подключение GitHub* в **имя подключения** текстового поля. Пример:
...
1. Когда появится запрос, войдите в учетную запись GitHub. Затем выберите авторизовать для предоставления доступа к учетной записи VSTS. В случае успешного выполнения создается новая конечная точка службы.
...
1. Перейдите к **в очереди** вкладке **сборки и выпуска** > **построения** страницы в VSTS. Построения в очереди показывает ветви и фиксации, запустившей сборки:
...
## <a name="examine-the-vsts-devops-pipeline"></a>Изучите конвейера VSTS DevOps
...
1. **Публикация** &mdash; интерфейсов `dotnet publish --configuration release --output <local_path_on_build_agent>` команду, чтобы создать *ZIP-файл* файл с артефактами, которые должны быть развернуты. `--output` Параметр указывает расположение публикации *ZIP-файл* файл. Что расположение задается путем передачи [Предопределенная переменная](https://docs.microsoft.com/vsts/pipelines/build/variables) с именем `$(build.artifactstagingdirectory)`. Эту переменную при развертывании по локальному пути, такие как *c:\agent\_work\1\a*, на агенте построения.
...
Конвейер выпуска состоит из двух задач: *развернуть службу приложений Azure для слота* и *управление службой приложений Azure — для переключения слотов*. Щелкнув первая задача раскрывает конфигурация следующие задачи:
...
Предоставляются подписки, группы ресурсов, тип службы, имя веб-приложения и сведения о слот развертывания. **Переключить на рабочий слот** флажок. Следовательно, биты развертывания *промежуточных* меняются местами слот в рабочую среду.

## <a name="additional-reading"></a>Дополнительные материалы для чтения

* [Сборка приложения ASP.NET Core](https://docs.microsoft.com/vsts/build-release/apps/aspnet/build-aspnet-core)
* [Создание и развертывание в веб-приложение Azure](https://docs.microsoft.com/vsts/build-release/apps/cd/azure/aspnet-core-to-azure-webapp)
* [Определение процесса сборки CI для репозитория GitHub](https://docs.microsoft.com/vsts/pipelines/build/ci-build-github)
