...
1. **Обзор** вкладка отображает полезные сведения «в краткие», включая диаграммы, отображения последних метрик.
    ...
    * **HTTP 5xx**: Число ошибок на стороне сервера, как правило, исключения в ASP.NET Core коде.
    * **Данные в**: Данные, поступающие в веб-приложение.
    * **Исходящие данные**: Исходящие данные из веб-приложения на клиенты.
    * **Запросы**: Число HTTP-запросов.
    * **Среднее время**отклика: Среднее время, в течение которого веб-приложение будет отвечать на запросы HTTP.
...
> *Примечание. Сейчас оповещение о метриках веб-приложения доступно только в службе "оповещения (классическая)".*
