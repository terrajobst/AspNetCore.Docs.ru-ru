...
Привязка данных как к компонентам, так и к элементам DOM осуществляется с помощью атрибута [@bind](xref:mvc/views/razor#bind) . В следующем примере выполняется привязка поля `_italicsCheck` к установленному флажку:
