...
Интерактивные компоненты с отслеживанием состояния могут быть добавлены на страницу Razor или в представление. При отрисовке страницы или представления компонент предварительно отображает его. После установки клиентского подключения приложение снова подключится к состоянию компонента, пока состояние остается в памяти.
 
Например, Следующая страница Razor визуализирует `Counter` компонент с начальным счетчиком, указанным с помощью формы:
