...
Скрипт &dagger;The *блазор. Server. js* обслуживается из внедренного ресурса в ASP.NET Core общей платформе.
...
* `components-reconnect-hide` &ndash; клиент имеет активное подключение, скрыть пользовательский интерфейс.
...
`RenderMode` — настраивается ли компонент:
...
Чтобы избежать сценария двойной отрисовки в серверном приложении Блазор:
...
В следующем коде показано обновленное `WeatherForecastService` в серверном приложении Блазор на основе шаблона, которое позволяет избежать двойной отрисовки:
...
На следующей странице Razor отображается компонент `Counter`:
...
На следующей странице Razor компонент `MyComponent` статически отображается с начальным значением, указанным с помощью формы:
