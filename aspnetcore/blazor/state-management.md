...
> [!NOTE]
> "На стороне клиента" в этом разделе относится к сценариям на стороне клиента в браузере, а не к [модели размещения веб-сборки блазор](xref:blazor/hosting-models#blazor-webassembly). `localStorage`и `sessionStorage` могут использоваться в приложениях блазор веб-сборки, но только путем написания пользовательского кода или использования стороннего пакета.
...
Чтобы отключить предварительную отрисовку, откройте файл *pages/_Host. cshtml* и измените вызов `Html.RenderComponentAsync<App>(RenderMode.Server)`.
