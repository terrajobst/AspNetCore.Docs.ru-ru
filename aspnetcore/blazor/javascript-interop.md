...
Для приложений на стороне сервера:

* Приложение на стороне сервера обрабатывает несколько запросов пользователей. Не вызывайте `JSRuntime.Current` в компоненте для вызова функций JavaScript.
...
Внутри элемента wwwroot/index.HTML ( блазор на стороне клиента) или Pages */_Host. cshtml* (блазор на стороне сервера) Укажите функцию, которая использует `TextDecoder` для декодирования переданного массива: `<head>`
...
В примере клиентского приложения, прилагаемого к этой теме, для клиентского приложения, которое взаимодействует с моделью DOM для получения вводимых пользователем данных и вывода приветственного сообщения, доступны две функции JavaScript.
...
Поместите тег, который ссылается на файл JavaScript, в файл wwwroot/index.HTML (на стороне клиента блазор) или *pages/_Host. cshtml* (блазор на стороне сервера). `<script>`

*wwwroot/index.HTML* (На стороне клиента блазор):

[!code-html[](./common/samples/3.x/BlazorSample/wwwroot/index.html?highlight=15)]

*Pages/_Host. cshtml* (Блазор на стороне сервера):
...
`IJSRuntime` Абстракция является асинхронной, чтобы разрешить сценарии на стороне сервера. Если приложение работает на стороне клиента и требуется вызвать функцию JavaScript синхронно, следует использовать приведение к `IJSInProcessRuntime` и вызов. `Invoke<T>` Рекомендуется, чтобы большинство библиотек взаимодействия JavaScript использовали асинхронные интерфейсы API, чтобы обеспечить доступность библиотек во всех сценариях, на стороне клиента или на стороне сервера.
...
* Передайте экземпляр .NET в JavaScript, заключив его в `DotNetObjectRef` экземпляр. Экземпляр .NET передается по ссылке на JavaScript.
