...
Файл *web.config* в первую очередь настраивает модуль ASP.NET Core. Он может также предоставлять дополнительные параметры конфигурации IIS. За создание, преобразования и публикацию *web.config* отвечает веб-пакет SDK для .NET Core (`Microsoft.NET.Sdk.Web`). Пакет SDK устанавливается поверх файла проекта (*CSPROJ*), `<Project Sdk="Microsoft.NET.Sdk.Web">`. Чтобы пакет SDK не преобразовывал файл *web.config*, добавьте в файл проекта свойство **\<IsTransformWebConfigDisabled>** со значением `true`:
