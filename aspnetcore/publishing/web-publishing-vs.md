...
Убедитесь, что приложение, публикуемое для развертывания, не запущено. Когда приложение выполняется, файлы в папке *publish* блокируются. Заблокированные файлы скопировать нельзя, так что развертывание в этом случае не произойдет.
...
В настоящее время `dotnet publish` не позволяет использовать профили публикации. Для работы с профилями публикации используйте `dotnet build`. `dotnet build` вызывает MSBuild в проекте. Кроме того, `msbuild` можно вызвать напрямую.
