...
* Удалите атрибут «Version» на ссылку на пакет `Microsoft.AspNetCore.App`. Проекты, использующие `<Project Sdk="Microsoft.NET.Sdk.Web">` не нужно задавать версии. Версия содержится в разрешении требуемой версии .NET framework и выбран для наиболее подходящий способ работы в ASP.NET Core 2.1. Дополнительные сведения см. в разделе [правила для проектов, предназначенных для общей платформы](#rules-for-projects-targeting-the-shared-framework) раздел.
...
* Обновление  *\_LoginPartial.cshtml* файла:
  ...
  * Обновление `<form />` элемент `<form asp-area="Identity" asp-page="/Account/Logout" asp-route-returnUrl="@Url.Page("/Index", new { area = "" })" method="post" id="logoutForm" class="navbar-right">`.
...
* В *области/Identity/страниц/\_ViewStart.cshtml*, изменить `Layout = "/Pages/_Layout.cshtml"` для `Layout = "/Pages/Shared/_Layout.cshtml"`.
* *\_Layout.cshtml* файл имеет следующие изменения:
