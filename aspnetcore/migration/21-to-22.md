...
Если приложение вызывает <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> в `CreateDefaultBuilder` метод `Program` вызовите `ConfigureKestrel` Настройка сервере Kestrel, а не `UseKestrel` во избежание конфликтов с [IIS в процессе модель размещения](xref:fundamentals/servers/index#in-process-hosting-model):
...
Если приложение не вызывает метод `CreateDefaultBuilder`, вызовите <xref:Microsoft.AspNetCore.Hosting.WebHostBuilderKestrelExtensions.UseKestrel*> **перед** вызова `ConfigureKestrel`:
