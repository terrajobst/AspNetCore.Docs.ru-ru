...
* Если приложение получает прямой доступ к `RouteData.Routers` в MVC для анализа URL-адресов, вы можете заменить это с помощью [линкпарсер. парсепасбендпоинтнаме](xref:Microsoft.AspNetCore.Routing.LinkParserEndpointNameAddressExtensions.ParsePathByEndpointName*). 
...
Как описано в документации по [различиям между предыдущими версиями маршрутизации](xref:fundamentals/routing#differences-from-earlier-versions-of-routing), существуют некоторые различия в создании ссылок (например, с помощью `Url.Link` и аналогичных интерфейсов API). К ним относятся следующие объекты.
