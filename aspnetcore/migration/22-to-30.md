...
В 3,0 компиляция среды выполнения является сценарием согласия. Сведения о включении компиляции среды выполнения см. в разделе <xref:mvc/views/view-compilation#runtime-compilation>.
