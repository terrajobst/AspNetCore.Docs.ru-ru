...
* Если приложение использует функции проверки подлинности и авторизации `AuthorizePage` , `[Authorize]`такие как или, поместите `UseAuthentication` вызов `UseAuthorization` в и **после** `UseRouting`.
* Если приложение использует функции [CORS](xref:security/cors) , такие как `[EnableCors]`, поместите `UseCors` следующий.
