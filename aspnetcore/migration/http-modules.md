...
1. [Жизненный цикл приложения](https://msdn.microsoft.com/library/ms227673.aspx), который представляет собой ряд событий, запускаемых ASP.NET: [BeginRequest](/dotnet/api/system.web.httpapplication.beginrequest), [AuthenticateRequest](/dotnet/api/system.web.httpapplication.authenticaterequest)и т. д. Каждый модуль может создать обработчик для одного или нескольких событий.
...
3. Связывание значений параметров с классом Options
    ...
    Шаблон параметров использует инфраструктуру внедрения зависимостей ASP.NET Core для связывания типа параметров (например, `MyMiddlewareOptions`) `MyMiddlewareOptions` с объектом с фактическими параметрами.
