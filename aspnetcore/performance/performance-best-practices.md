...
В этом документе *ветви кода* определяется как путь кода, который часто называют и где большую часть времени выполнения происходит. "Горячий" путей обычно ограничивают приложения масштабирования и производительности.
...
Распространенные проблемы с производительностью в приложениях ASP.NET Core блокирует вызовы, которые могут выполняться асинхронно. Многие синхронных вызовов блокировки привести к [нехватка потоков пула](https://blogs.msdn.microsoft.com/vancem/2018/10/16/diagnosing-net-core-threadpool-starvation-with-perfview-why-my-service-is-not-saturating-all-cores-or-seems-to-stall/) и снижение времени ответа.
...
Проблем с памятью, например выше, можно диагностировать, просмотрев статистика сборки мусора (GC) коллекции в [PerfView](https://github.com/Microsoft/perfview) и изучив:
...
Взаимодействия с хранилищем данных и других удаленных служб часто являются медленных процессов приложения ASP.NET Core. Чтение и запись данных эффективно крайне важно для высокой производительности.
...
* **Сделать** рассмотрите возможность кэширования часто используемых данных, полученных из базы данных или удаленной службы, если допускается несколько устаревшие данные. В зависимости от сценария, используйте [MemoryCache](xref:performance/caching/memory) или [DistributedCache](xref:performance/caching/distributed). Дополнительные сведения см. в разделе <xref:performance/caching/response>.
* **Сделать** свести к минимуму сетевой циклами. Цель состоит в извлечении необходимых данных в одном вызове, а не несколько вызовов.
...
* **Сделать** фильтра и объединение запросов LINQ (с `.Where`, `.Select`, или `.Sum` инструкций, например), чтобы фильтрация выполняется в базе данных.
* **Сделать** учитывать, что EF Core разрешает некоторые операторы запроса на клиенте, что может привести к выполнению неэффективным. Дополнительные сведения см. в разделе [проблем с производительностью клиента оценки](/ef/core/querying/client-eval#client-evaluation-performance-issues).
...
Мы рекомендуем оценка важности выше подходы высокой производительности перед фиксацией базы кода. Дополнительные сложности скомпилированных запросов может не оправдать повышение производительности.
...
Необходимо, что весь код, fast, часто вызываемый пути кода наиболее важны для оптимизации:
...
* **Сделать** использовать средства профилирования, таких как производительность [средств диагностики Visual Studio](/visualstudio/profiling/profiling-feature-tour) или [PerfView](https://github.com/Microsoft/perfview)), чтобы определить [горячего пути кода](#hot).
...
* **Сделать** рассмотрите возможность обработки длительных запросов с [фоновых служб](xref:fundamentals/host/hosted-services) или вне процесса с [Azure функция](/azure/azure-functions/). Завершение работы вне процесса особенно удобна для задач, интенсивно использующие ЦП.
* **Сделать** использовать параметры взаимодействий в реальном времени, такие как [SignalR](xref:signalr/introduction), для асинхронного взаимодействия с клиентами.
...
* Уменьшения, что уменьшает размер файлов, удаляя пробелы и комментарии.
...
Каждым новым выпуском служб ASP.NET Core включает в себя повышение производительности. Оптимизации в .NET Core и ASP.NET Core означает, что более новые версии обычно более эффективны, чем предыдущие версии. Например, .NET Core 2.1 добавлена поддержка для скомпилированные регулярные выражения и использует преимущества [ `Span<T>` ](https://msdn.microsoft.com/magazine/mt814808.aspx). ASP.NET Core 2.2 добавлена поддержка HTTP/2. Если производительность важна, следует обновить до текущей версии ASP.NET Core.
...
* **Не** использование, создание или перехват исключений с точки зрения нормального выполнения программы, особенно в [горячего пути кода](#hot).
...
Средства диагностики приложений, таких как Application Insights может помочь определить общие исключения в приложении, которое может повлиять на производительность.
