...
Используйте распределенный кэш для хранения данных в памяти, когда приложение размещается в облаке или сервер фермы. Кэш распределяется по всем серверам, которые обрабатывают запросы. Клиент может отправки нет доступных запросов, обрабатываемых любой сервер в группе и кэшированных данных для клиента. ASP.NET Core предлагает SQL Server и Redis распределенного кэша.
