...
Распределенный кэш является общим для нескольких серверов приложений (см. [основы кэша](memory.md#caching-basics)). Информация в кэше не хранится в памяти отдельных веб-серверов, и кэшированные данные доступны для всех серверов приложений. Это обеспечивает несколько преимуществ:
...
Для использования интерфейса `IDistributedCache`:
   ...
   3. Из классов [промежуточного уровня](xref:fundamentals/middleware/index) или MVC-контроллера приложения запросите экземпляр `IDistributedCache` в конструкторе. Экземпляр будет предоставлен через [внедрение зависимостей](../../fundamentals/dependency-injection.md) (DI).
