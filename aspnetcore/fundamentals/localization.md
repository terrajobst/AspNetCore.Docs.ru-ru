...
Используйте `IHtmlLocalizer<T>` реализации для ресурсов, содержащие HTML. `IHtmlLocalizer`Кодирует строку ресурса форматируемые аргументы HTML, но не кодировка HTML собственно строкой ресурса. В примере выделен под только значение `name` параметр — в кодировке HTML.
