...
Если необходимо задать переменные среды для отдельных приложений, выполняющихся в изолированной пулы приложений (поддерживается в IIS 10.0 +), см. раздел *команды AppCmd.exe* раздел [переменных среды \< environmentVariables >](/iis/configuration/system.applicationHost/applicationPools/add/environmentVariables/#appcmdexe) раздел в IIS справочной документации.
...
* [Общие сведения о вспомогательных функций тегов](../mvc/views/tag-helpers/intro.md)
