...
Ведение журнала во время создания узла не поддерживается напрямую. Однако можно использовать отдельное средство ведения журнала. В следующем примере для входа в [ используется средство ведения журнала ](https://serilog.net/)Serilog`CreateHostBuilder`. `AddSerilog` использует статическую конфигурацию, указанную в `Log.Logger`.
...
Ведение журнала во время создания узла не поддерживается напрямую. Однако можно использовать отдельное средство ведения журнала. В следующем примере для входа в [ используется средство ведения журнала ](https://serilog.net/)Serilog`CreateWebHostBuilder`. `AddSerilog` использует статическую конфигурацию, указанную в `Log.Logger`.
...
| Number | Поставщик      | Категории, которые начинаются с...          | Минимальный уровень ведения журнала |
| :----: | ------------- | --------------------------------------- | ----------------- |
...
| 8      | Отладка         | Microsoft                               | Трассировка             |
