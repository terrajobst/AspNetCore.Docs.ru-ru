...
> [!NOTE]
> Связывание конечных точек ограничено действиями и страницами MVC и Razor Pages. Расширение функций связывания конечных точек планируется в будущих выпусках.
...
* Конечная точка маршрута содержит шаблон, метаданные и делегат запроса, который обслуживает ответ конечной точки. Метаданные используются для реализации сквозной функциональности на основе политик и конфигурации каждой конечной точки. Например, ПО промежуточного слоя авторизации может запросить [политику авторизации](xref:security/authorization/policies#applying-policies-to-mvc-controllers) у коллекции метаданных конечной точки.
* Маршрутизация конечной точки интегрируется с ПО промежуточного слоя с использованием двух приведенных ниже методов расширения.
  * [UseRouting](xref:Microsoft.AspNetCore.Builder.EndpointRoutingApplicationBuilderExtensions.UseRouting*) добавляет соответствие маршрута в конвейер ПО промежуточного слоя. Он должен быть указан перед любым ПО промежуточного слоя, учитывающим маршрут, таким как авторизация, выполнение конечной точки и т. д.
  * [UseEndpoints](xref:Microsoft.AspNetCore.Builder.EndpointRoutingApplicationBuilderExtensions.UseEndpoints*) добавляет выполнение конечной точки в конвейер ПО промежуточного слоя. Он запускает делегат запроса, который обслуживает ответ конечной точки.
  В `UseEndpoints` также настраиваются конечные точки маршрута, которые могут быть сопоставлены и выполнены приложением. Например, <xref:Microsoft.AspNetCore.Builder.RazorPagesEndpointRouteBuilderExtensions.MapRazorPages*>, <xref:Microsoft.AspNetCore.Builder.ControllerEndpointRouteBuilderExtensions.MapControllers*>, <xref:Microsoft.AspNetCore.Builder.EndpointRouteBuilderExtensions.MapGet*> и <xref:Microsoft.AspNetCore.Builder.EndpointRouteBuilderExtensions.MapPost*>.
* Приложения используют вспомогательные методы ASP.NET Core для настройки маршрутов. Платформы ASP.NET Core предоставляют такие вспомогательные методы, как <xref:Microsoft.AspNetCore.Builder.RazorPagesEndpointRouteBuilderExtensions.MapRazorPages*>, <xref:Microsoft.AspNetCore.Builder.ControllerEndpointRouteBuilderExtensions.MapControllers*> и `MapHub<THub>`. Существуют также вспомогательные методы для настройки собственных конечных точек маршрута: <xref:Microsoft.AspNetCore.Builder.EndpointRouteBuilderExtensions.MapGet*>, <xref:Microsoft.AspNetCore.Builder.EndpointRouteBuilderExtensions.MapPost*> и [MapVerb](xref:Microsoft.AspNetCore.Builder.EndpointRouteBuilderExtensions). 
* Маршрутизация конечных точек также поддерживает изменения в конечных точках после запуска приложения. Для этого в приложении или платформе ASP.NET Core необходимо создать и зарегистрировать пользовательскую функцию <xref:Microsoft.AspNetCore.Routing.EndpointDataSource>. Это дополнительная функция, которая обычно не нужна. Конечные точки обычно настраиваются при запуске и являются фиксированными в течение всего срока жизни службы приложения. Загрузка конфигурации маршрута из файла или базы данных при запуске не является динамической.

В следующем коде приведен базовый пример маршрутизации конечной точки:

[!code-csharp[](routing/samples/3.x/Startup.cs?name=snippet)]

Дополнительные сведения см. в разделе [Соответствие URL-адресов](#url-matching).
