...
* Создает скрытый [токена проверки запроса](http://www.asp.net/mvc/overview/security/xsrfcsrf-prevention-in-aspnet-mvc-and-web-pages) для предотвращения подделки межсайтовых запросов (при использовании с `[ValidateAntiForgeryToken]` атрибута в методе действия HTTP Post)
...
Множество представлений в *представления и учетная запись* папку (созданы при создании нового веб-приложения с *отдельных учетных записей пользователей*) содержать [asp маршрута returnurl](http://docs.asp.net/en/latest/mvc/views/working-with-forms.html#the-form-tag-helper) атрибут:
...
* Задает HTML- `type` атрибута значение в зависимости от типа модели и [заметки к данным](https://msdn.microsoft.com/library/system.componentmodel.dataannotations.aspx) атрибуты, примененные к свойству модели

* Не перезаписывает HTML `type` значение атрибута, если он указан

* Приводит к возникновению ошибки [HTML5](https://developer.mozilla.org/docs/Web/Guide/HTML/HTML5) атрибуты проверки из [заметки к данным](https://msdn.microsoft.com/library/system.componentmodel.dataannotations.aspx) атрибуты, примененные к свойства модели
...
В следующей таблице приведены некоторые наиболее распространенные [заметок к данным](https://msdn.microsoft.com/library/system.componentmodel.dataannotations.aspx) атрибуты, сопоставленные вспомогательный тега входных данных для определенного типа входных данных (не каждый атрибут проверки присутствует):
...
* Приводит к возникновению ошибки `id` и `name` атрибуты и атрибуты проверки данных из модели для [ \<textarea >](http://www.w3.org/wiki/HTML/Elements/textarea) элемента.
...
* [Вспомогательных функций тегов](tag-helpers/intro.md)

* [Элемент HTML-формы](https://www.w3.org/TR/html401/interact/forms.html)

* [Маркер проверки запроса](http://www.asp.net/mvc/overview/security/xsrfcsrf-prevention-in-aspnet-mvc-and-web-pages)

* [Привязка модели](../models/model-binding.md)

* [Проверка модели](../models/validation.md)

* [заметок к данным](https://msdn.microsoft.com/library/system.componentmodel.dataannotations.aspx)
