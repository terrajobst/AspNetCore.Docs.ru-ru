...
В ASP.NET Core MVC, представления, *.cshtml* файлы, использующие [языка программирования C#](/dotnet/csharp/) в разметке Razor. Как правило, просматривать файлы группируются в папки для каждого приложения [контроллеров](xref:mvc/controllers/actions). Папки, хранятся в в *представления* папку в корне приложения:
...
* Компоненты приложения не являются тесно связанными. Можно создать и обновить представления приложения отдельно от бизнес-логику и данные компоненты доступа. Представления приложения можно изменить, не имея обновления других частей приложения.
...
Для работы с проектами, такие как [Visual Studio](https://www.visualstudio.com/vs/) или [кода Visual Studio](https://code.visualstudio.com/), можно составить список членов (свойства модели) при добавлении их представление, которое помогает писать код быстрее и с меньшим количеством ошибок. Эта возможность называется [IntelliSense](/visualstudio/ide/using-intellisense) в набор средств Microsoft.
...
Оба `ViewData` и `ViewBag` не менее допустимые подходы для передачи небольших объемов данных между контроллерами и представлениями. Нужный вариант один (или оба) зависит от предпочтений или копирования для вашей организации. Как правило разработчики используют одно из них, согласованы. Либо используйте `ViewData` everywhere или использовать `ViewBag` везде, но можно комбинировать и сопоставлять их. Поскольку оба динамически разрешенных во время выполнения и таким образом подвержена приводит к ошибкам времени выполнения, внимательно используйте их. Некоторые разработчики полностью избежать подобных проблем.
