...
Существует несколько подходов к передаче данных в представления. Наиболее надежный подход — указание типа [модели](xref:mvc/models/model-binding) в представлении. Такая модель называется *viewmodel*. Экземпляр типа viewmodel передается в представление из действия.
...
### <a name="weakly-typed-data-viewdata-and-viewbag"></a>Нестрого типизированные данные (ViewData и ViewBag)
