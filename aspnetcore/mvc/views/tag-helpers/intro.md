...
Visual Studio можно ввести *CompleteWord* ярлык (Ctrl + Пробел является [по умолчанию](https://msdn.microsoft.com/library/da5kh0wa.aspx)) внутри двойных кавычек ("»), и теперь вы находитесь в C#, так же, как будет находиться в классе C#. IntelliSense отображает все методы и свойства по модели страницы. Методы и свойства недоступны, так как тип свойства `ModelExpression`. На рисунке ниже, я редактирования `Register` представления, поэтому `RegisterViewModel` доступен.
...
В (`@`) символ дает Razor находится в начале кода. Следующие два параметра («FirstName» и «имя:») представляют собой строки, поэтому [IntelliSense](https://msdn.microsoft.com/library/hcw1s69b.aspx) не может им помочь. Последний аргумент:
