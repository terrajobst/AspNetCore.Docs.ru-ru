...
---
# <a name="introduction-to-tag-helpers-in-aspnet-core"></a>Общие сведения о вспомогательных функциях тегов в ASP.NET Core 
...
Большинство встроенных вспомогательных функций тегов работают с существующими HTML-элементами и предоставляют для них атрибуты на стороне сервера. Например, элемент `<input>`, используемый во многих представлениях в папке *Views/Account*, содержит атрибут `asp-for`, который извлекает имя указанного свойства указанной модели в отрисованный HTML. Следующая разметка Razor:
...
Атрибут `asp-for` доступен для свойства `For` в функции `LabelTagHelper`. Дополнительные сведения см. в статье [Создание вспомогательных функций тегов](authoring.md).
...
Символ at (`@`) указывает Razor, что это начало кода. Следующие два параметра ("FirstName" и "First Name:") представляют собой строки, поэтому [IntelliSense](https://docs.microsoft.com/visualstudio/ide/using-intellisense) помочь здесь не может. Последний аргумент:
...
* Несколько вспомогательных функций тегов могут работать с одним и тем же элементом (см. статью о [предотвращении конфликтов вспомогательной функции тега](https://docs.microsoft.com/aspnet/core/mvc/views/tag-helpers/authoring#avoiding-tag-helper-conflicts)), серверные веб-элементы управления обычно нельзя комбинировать.
