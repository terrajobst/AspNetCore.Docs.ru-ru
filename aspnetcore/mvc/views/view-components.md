...
Новое в ASP.NET Core MVC, просмотр компонентов похожи на частичного представления, но они более мощными. Просмотр компонентов не использовать привязки модели и только зависят от данных, которые предоставляют при вызове в него. Компонент представления:
...
* Не принимают участие в жизненном цикле контроллер, то есть нельзя использовать [фильтры](../controllers/filters.md) в компоненте представления
...
* Не доступны непосредственно как конечную точку HTTP, их вы вызывать из кода приложения (обычно в представление). Компонент представления никогда не обрабатывается запрос
...
Просмотр компонентов обычно вызываются из представления, но их можно вызывать непосредственно из метода контроллера. Если Просмотр компонентов не определяют конечных точек, например контроллеров, можно легко реализовать действия контроллера, который возвращает содержимое `ViewComponentResult`.
...
* `[ViewComponent]` Атрибута можно изменить имя, используемое для ссылки на компонент представления. Например, нам удалось назвали класс `XYZ` и применяется `ViewComponent` атрибута:
...
* Создание *Views/Shared/Components/PriorityList/Default.cshtml* представления Razor:[!code-cshtml[Main](view-components/sample/ViewCompFinal/Views/Shared/Components/PriorityList/Default1.cshtml)]
...
Если при отображении представления PVC, убедитесь, что вы вызываете компонент представления с приоритетом 4 или более поздней версии.
