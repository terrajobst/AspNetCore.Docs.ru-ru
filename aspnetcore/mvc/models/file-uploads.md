...
Для элемента ввода `files`, поддерживающего отправку нескольких файлов, в элементе `multiple` необходимо указать атрибут `<input>`:
...
В классы `FileSizeLimit` внедряется `PageModel`.
