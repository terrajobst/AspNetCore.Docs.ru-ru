...
Можно также обновить правила проверки в форме, когда отдельные элементы управления, такие как `<input/>`s и `<select/>`s, создаются динамически. Невозможно передать селекторы для этих элементов для `parse()` непосредственно из-за форму уже проверен и не обновляются. Вместо этого сначала удалите существующие данные проверки, и повторной обработки всей формы, как показано ниже:
