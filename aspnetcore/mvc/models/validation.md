...
Не допускающий [типы значений](/dotnet/csharp/language-reference/keywords/value-types) (такие как `decimal`, `int`, `float`, и `DateTime`) по своей природе требуются и не нужен `Required` атрибута. Приложение не выполняет проверки на стороне сервера проверки для запретом типов, помеченных `Required`.
