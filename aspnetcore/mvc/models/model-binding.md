...
* Типы, допускающие значения NULL. Типам, допускающим значение NULL, задается значение `null`. В приведенном выше примере привязка модели задает `id` значение `null`, так как он имеет тип `int?`.
...
* `[FromBody]`: используйте настроенные форматировщики для привязки данных из текста запроса. Модуль форматирования выбирается в зависимости от типа содержимого запроса.
