...
При добавлении функций SignalR в ASP.NET Core приложение настройте маршруты SignalR, вызвав `endpoint.MapHub` в обратном вызове `app.UseEndpoints` метода `Startup.Configure`.
...
При добавлении функций SignalR в ASP.NET Core приложение настройте маршруты SignalR, вызвав `app.UseSignalR` в методе `Startup.Configure`.
...
Использование строго типизированных `Hub<T>` отключает возможность использования `SendAsync`. Все методы, определенные в интерфейсе, по-прежнему могут быть определены как асинхронные. Фактически каждый из этих методов должен возвращать `Task`. Поскольку это интерфейс, не используйте ключевое слово `async`. Пример:
