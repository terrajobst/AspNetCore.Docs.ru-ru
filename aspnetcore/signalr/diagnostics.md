...
* `Microsoft.AspNetCore.SignalR` &ndash; для журналов, связанных с протоколы концентратора активация концентраторов, вызов методов и других действий, связанных с центром.
* `Microsoft.AspNetCore.Http.Connections` &ndash; для журналов, связанных с транспортов, таких как WebSockets, длинный опрос Server-Sent событий и низкоуровневой инфраструктурой SignalR.
...
Включить **ведение журнала приложения (файловая система)** в диалоговом окне **журналы диагностики** разделе портала службы приложений Azure и настроить **уровень** для `Verbose`. Журналы должны быть доступны из **потоковая передача журналов** службы и в журналах файловой системе службы приложений. Дополнительные сведения см. в разделе [потоковая передача журналов Azure](xref:fundamentals/logging/index#azure-log-streaming).

### <a name="other-environments"></a>Другие среды

Если приложение развертывается в другую среду (например, Docker, Kubernetes или службы Windows), см. в разделе <xref:fundamentals/logging/index> Дополнительные сведения о том, как настроить поставщики ведения журналов для среды.
...
> [!NOTE]
> Пожалуйста не вставьте содержимое файлов журнала и трассировки сети в вопроса в GitHub. Эти журналы и трассировки могут быть довольно большими, и GitHub обычно усекает их.
