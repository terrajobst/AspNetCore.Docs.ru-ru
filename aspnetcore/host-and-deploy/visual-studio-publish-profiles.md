...
[Публикации dotnet](/dotnet/core/tools/dotnet-publish) команда вызывает MSBuild, который вызывает `Publish` цели. Любые параметры, передаваемые в `dotnet publish` передаются в MSBuild. Параметр `-c` сопоставляется со свойством `Configuration` MSBuild. Параметр `-o` сопоставляется со свойством `OutputPath`.
