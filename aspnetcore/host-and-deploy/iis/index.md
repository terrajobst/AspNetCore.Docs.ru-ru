...
2. В папке создайте папку *logs* для хранения журналов stdout, если включено ведение журнала stdout. Если приложение развертывается с папкой *logs* в полезных данных, пропустите этот шаг. Инструкции о том, как MSBuild создает папку *logs*, см. в разделе [Структура каталога](xref:host-and-deploy/directory-structure).
