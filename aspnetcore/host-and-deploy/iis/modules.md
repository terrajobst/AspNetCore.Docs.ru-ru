...
Приложения ASP.NET Core размещаются в службах IIS в конфигурации обратного прокси-сервера. Некоторые собственные модули IIS и все управляемые модули IIS не могут применяться для обработки запросов в приложениях ASP.NET Core. Во многих случаях функции собственных и управляемых модулей IIS можно переложить на ASP.NET Core.

## <a name="native-modules"></a>Собственные модули

В этой таблице перечислены собственные модули IIS, которые доступны в приложениях ASP.NET Core при обработке запросов обратного прокси-сервера.
...
| **Трассировка**<br>`FailedRequestsTracingModule` | Да | [Ведение журналов ASP.NET Core](xref:fundamentals/logging/index#the-tracesource-provider) |
...
Многие модули поддерживают параметр конфигурации, позволяющий отключить их без удаления модуля из приложения. Это самый простой и быстрый способ отключить отдельный модуль. Например, вы можете отключить модуль перенаправления HTTP с помощью элемента **\<httpRedirect >** в файле *web.config*:
...
Если вы решите удалить модуль с помощью параметра в файле *web.config*, первым делом разблокируйте этот модуль и элемент  **\<modules>** в файле *web.config*:
