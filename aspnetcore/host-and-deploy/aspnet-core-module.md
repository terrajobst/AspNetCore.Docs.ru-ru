...
*Web.config* ниже приведен пример для [автономное развертывание](https://docs.microsoft.com/dotnet/articles/core/deploying/#self-contained-deployments-scd) для [службе приложений Azure](https://azure.microsoft.com/services/app-service/). Дополнительные сведения см. в разделе [узла в Windows с помощью IIS](xref:host-and-deploy/iis/index). В разделе [конфигурации дочерних приложений](xref:host-and-deploy/iis/index#sub-application-configuration) для важное примечание, относящиеся к конфигурации *web.config* файлы в дочерних приложений.
