...
*Web.config* ниже приведен пример для [автономное развертывание](https://docs.microsoft.com/dotnet/articles/core/deploying/#self-contained-deployments-scd) для [службе приложений Azure](https://azure.microsoft.com/services/app-service/). Дополнительные сведения см. в разделе [узла в Windows с помощью IIS](xref:host-and-deploy/iis/index). В разделе [конфигурации дочерних приложений](xref:host-and-deploy/iis/index#configuration-of-sub-applications) для важное примечание, относящиеся к конфигурации *web.config* файлы в дочерних приложений.
...
| Атрибут | Описание: |
| --- | --- |
| processPath | <p>Обязательный строковый атрибут.</p><p>Путь к исполняемому файлу, который запустит процесс, прослушивающий для HTTP-запросов. Поддерживаются относительные пути. Если путь начинается с ".", путь считается относительно корневого каталога сайта.</p><p>Нет значения по умолчанию.</p> |
| arguments | <p>Необязательный строковый атрибут.</p><p>Аргументы для исполняемого файла, указанного в **processPath**.</p><p>Значение по умолчанию - пустая строка.</p> |
...
```
В разделе [конфигурации через web.config](#configuration-via-webconfig) пример `aspNetCore` элемент в *web.config* файла.

## <a name="aspnet-core-module-with-an-iis-shared-configuration"></a>Модуль ASP.NET Core с IIS общей конфигурации

Модуль ASP.NET Core программа установки запускается с правами **системы** учетной записи. Так как учетная запись локальной системы не имеют права на изменение для путь к общей папке, которая используется общей конфигурации IIS, установщик столкнутся отказ в доступе при попытке настроить параметры модуля в  *applicationHost.config* в общей папке.
